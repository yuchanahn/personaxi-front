const e=`# Licenses & Third-Party Credits

PersonaXI ("the Company") utilizes the following open-source software and technologies in its Service.

---

## 1. Frontend (MIT License)
- Svelte / SvelteKit
- Three.js
- @pixiv/three-vrm
- Tailwind CSS
- Bits UI
- Iconify
- Marked
- Socket.io-client
- Supabase JS
- PixiJS / pixi-live2d-display

## 2. Backend

| Software | License |
|:---|:---|
| Go (Golang) | BSD 3-Clause |
| Chi Router | MIT License |
| Pgx | MIT License |
| Godotenv | MIT License |
| Gorilla WebSocket | BSD 2-Clause |
| Chromem-go | Apache License 2.0 ※ |

> **※ Apache 2.0 NOTICE Obligation:** Chromem-go is distributed under the Apache License 2.0. This license includes an obligation to retain the original copyright holder's NOTICE file with any distribution. The Company maintains the relevant NOTICE in the "Third-Party Notices" section of the Service.

## 3. Proprietary Technology — Live2D®

This Service incorporates proprietary technology from Live2D Inc.

- **Live2D Cubism SDK:** Proprietary License (Live2D Open Software License)
  - Copyright (c) Live2D Inc. All rights reserved.
  - Live2D and Cubism are registered trademarks of Live2D Inc.

## 4. Assets and Fonts

- **Typography:** Pretendard (SIL Open Font License 1.1)
- **3D Models (VRM):** Default characters are created by the Company or provided under permitted licenses. VRM models uploaded by users are the property of their respective creators. The Company holds no ownership over user-uploaded assets.

---

## License Summary

| License | Commercial Use | Modification | Key Conditions |
|:---|:---|:---|:---|
| MIT License | ✅ | ✅ | Copyright notice must be retained |
| BSD 2-Clause | ✅ | ✅ | Copyright notice must be retained |
| BSD 3-Clause | ✅ | ✅ | Copyright notice must be retained; product name usage restrictions apply |
| Apache 2.0 | ✅ | ✅ | NOTICE file must be retained |
| SIL OFL 1.1 | ✅ | ✅ | A separate license is required if the font name is changed |

---

## DMCA and Copyright Infringement

If you believe that any content on the Service infringes upon your copyright, please submit a notice to **support@personaxi.com** with the following information:

1. **Identification** of the copyrighted work that you claim has been infringed (or, if multiple works on a single site are covered by a single notification, a representative list of such works).
2. **Sufficient information** to locate the infringing material on the Service (e.g., URL or description of the location).
3. **Your contact information**, including your name, email address, and phone number.
4. A statement that you have a **good faith belief** that the use of the material in question is not authorized by the copyright owner, its agent, or applicable law.
5. A statement that you are **representing under penalty of perjury** that you are the copyright owner or are authorized to act on behalf of the copyright owner.

The Company shall review the notice promptly and take appropriate action upon receipt.`;export{e as default};
