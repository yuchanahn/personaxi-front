...
> [!번역 필요]
> 이 문서의 최신 변경 사항은 현재 한국어만 반영되었습니다. 최종 단계에서 en/ja 동기화가 필요합니다.
...
## 5. 사용자 업로드 Live2D/VRM 에셋 고지

1. 본 서비스는 Live2D/VRM **렌더링 기술과 표시 플랫폼**을 제공하며, 회원이 업로드한 개별 모델의 저작권을 이전받지 않습니다.
2. 회원은 자신이 저작권자이거나, 서비스 내 공개·상업적 이용·실시간 표시형 서비스 제공에 필요한 권리를 보유한 경우에만 에셋을 업로드할 수 있습니다.
3. 회사는 서비스 제공을 위해 모델 데이터를 암호화된 방식으로 일시 전송·로드·캐싱·메모리 적재할 수 있으며, 이는 실시간 표시를 위한 기술적 처리입니다.
4. 전항의 기술적 처리는 모델 파일 자체의 판매, 양도, 재라이선스, 퍼블리싱 또는 2차 배포를 의미하지 않습니다.
5. 개별 에셋의 최종 사용 허용 범위는 원저작자/배포처 라이선스가 우선하며, 회원은 해당 조건을 직접 확인할 책임이 있습니다.
6. 권리 침해 신고 또는 합리적 침해 의심이 접수된 경우 회사는 해당 에셋 및 관련 캐릭터를 임시 차단하거나 삭제할 수 있습니다.
